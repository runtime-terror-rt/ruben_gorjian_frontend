...
export default function PostDetailsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-hidden">
      ...
      <div
        className="relative w-full max-w-2xl bg-[#ffffff]/90 border border-[#d9d4c9] rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col max-h-[90vh] overflow-hidden"
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 pb-4 shrink-0 transition-all">
          <div className="space-y-1">
            <h3 className="text-2xl font-bold text-[#14110c] tracking-tight flex items-center gap-2">
               Post Intelligence
            ...
            <div className="flex items-center gap-3 text-xs text-[#6b6b6b]">
               ...
               <span className="w-1 h-1 rounded-full bg-[#e6e1d8]" />
               <span className="flex items-center gap-1.5">
        ...
        >
          {loading ? (
             <div className="py-20 flex flex-col items-center gap-4">
                <div className="w-12 h-12 border-4 border-[#d9d4c9] border-t-amber-400 rounded-full animate-spin" />
                <span className="text-[#6b6b6b] animate-pulse">Analyzing Post Data...</span>
             </div>
          ) : !post ? (
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {isAdmin && (post.user || post.author) && (
                <div className="p-4 bg-[#b08d3e]/10 border border-[#b08d3e]/20 rounded-2xl flex items-center gap-4">
                   <div className="p-2.5 bg-[#b08d3e]/20 rounded-xl">
                      <UserIcon className="w-5 h-5 text-[#b08d3e]" />
                   </div>
                   <div className="flex-1 min-w-0">
                      <p className="text-xs text-[#b08d3e] font-bold uppercase tracking-widest">Post Origin</p>
                      <h5 className="text-sm font-medium text-[#14110c] truncate">
                         {post.user?.fullName || post.user?.name || post.author?.name || post.user?.email || "Managed Account"}
                      </h5>
                   </div>
                   {/* <Button variant="ghost" size="sm" className="text-[#b08d3e] hover:bg-[#b08d3e]/10 text-xs">
                      View Profile
                   </Button> */}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="rounded-2xl border border-[#d9d4c9] bg-[#faf8f3] overflow-hidden group relative min-h-[200px]">
                         ...
                         <div className={clsx(
                           ...
                           allMediaUrls.length > 1 ? "grid-cols-2" : "grid-cols-1"
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4 pointer-events-none">
                               <span className="text-xs text-[#14110c] font-medium drop-shadow-md">
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 gap-3">
                    {post.targets.map((target) => {
                       ...
                       return (
                         <div key={target.id} className="bg-[#e6e1d8]/30 border border-[#d9d4c9] hover:border-[#d9d4c9]/80 transition-all rounded-2xl p-4 flex items-center gap-4 group">
                            <div className={clsx("p-3 rounded-2xl bg-[#ffffff] group-hover:scale-110 transition-transform", color)}>
                               <Icon className="w-6 h-6" />
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2">
                                  <span className="font-bold text-[#14110c] text-sm">{target.platform}</span>
                                  {target.socialAccount && (
                                    <span className="px-2 py-0.5 rounded-lg bg-[#ffffff] text-[10px] text-[#6b6b6b] border border-[#d9d4c9]">
                            ...
                            <div className="flex flex-col items-end gap-2">
                               ...
                               {target.errorMessage && (
                                  <div className="flex items-center gap-1.5 text-rose-400 group/err relative">
                                     ...
                                     <div className="absolute right-0 bottom-full mb-2 w-48 p-2 bg-rose-950 border border-rose-500/50 rounded-lg text-[10px] text-[#14110c] opacity-0 group-hover/err:opacity-100 transition-opacity z-50">
                                        {target.errorMessage}
                                     </div>
                                  </div>
                               )}
        ...
        {/* Action Footer */}
        <div className="p-6 shrink-0 border-t border-[#d9d4c9] bg-[#faf8f3] flex flex-col sm:flex-row gap-3">
            <Button 
               variant="outline" 
               onClick={onClose} 
               className="border-[#d9d4c9] bg-transparent text-[#6b6b6b] hover:text-[#14110c] hover:bg-[#e6e1d8] flex-1 order-2 sm:order-1"
            >
               Close
            </Button>
