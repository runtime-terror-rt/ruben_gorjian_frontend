...
export default function AdminSettingsPage() {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: (payload: any) => apiPatch<UserSettingsResponse>("/api/user/settings", payload),
    onSuccess: (data) => {
  ...
  const changePasswordMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPasswordForm({ "current-password": "", "new-password": "", "confirm-password": "" });
    },
    onError: (err: Error) => {
  ...
  const applyGlobalMutation = useMutation({
    mutationFn: (payload: any) => apiPut<any>("/api/admin/publishing-routing/global", payload),
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-global-publishing-routing"] });
    },
    onError: (err: Error) => {
  ...
  // --- Layout Helper ---
  const avatarUrl = avatarPreview || (settingsQuery.data?.profile.avatar.url ? `${settingsQuery.data.profile.avatar.url}${settingsQuery.data.profile.avatar.version ? `?v=${settingsQuery.data.profile.avatar.version}` : ""}` : null);

  const serverScope = globalRoutingQuery.data?.scope ?? "USERS_ONLY";
  const effectiveApplyTo = userChosenScope ?? serverScope;
  const effectiveGlobalDefault = draftGlobalDefault ?? globalRoutingQuery.data?.globalDefault ?? {
  ...
  return (
    <div className="p-6 space-y-8">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-slate-400 font-medium">Manage your administrative profile and system-wide configurations.</p>
      </div>

      <Tabs defaultValue="profile" className="space-y-6">
        <TabsList className="bg-slate-900 border border-slate-800 p-1 h-12 rounded-2xl">
          <TabsTrigger value="profile" className="rounded-xl px-8 data-[state=active]:bg-lime-400 data-[state=active]:text-slate-950 font-bold transition-all">
            <User className="h-4 w-4 mr-2" /> Profile
          </TabsTrigger>
          <TabsTrigger value="security" className="rounded-xl px-8 data-[state=active]:bg-lime-400 data-[state=active]:text-slate-950 font-bold transition-all">
            <Lock className="h-4 w-4 mr-2" /> Security
          </TabsTrigger>
          <TabsTrigger value="system" className="rounded-xl px-8 data-[state=active]:bg-lime-400 data-[state=active]:text-slate-950 font-bold transition-all">
        ...
        <TabsContent value="profile" className="space-y-6">
          <Card className="bg-slate-900/40 border-slate-800 backdrop-blur-sm rounded-3xl overflow-hidden">
            <CardHeader className="border-b border-white/5 bg-white/5">
              ...
              <CardDescription className="text-slate-500">Your public identity within the management dashboard.</CardDescription>
            </CardHeader>
            <CardContent className="pt-8 space-y-8">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-white tracking-wide uppercase">Profile Photo</h4>
                  <p className="text-xs text-slate-500 max-w-[200px]">Allowed JPG, PNG or WEBP. Max size of 5MB.</p>
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={onRemoveAvatar} className="text-red-400 hover:text-red-300 hover:bg-red-500/10">
              ...
              <form onSubmit={handleProfileSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 space-y-2">
                  <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">Bio / About Me</Label>
                  <textarea 
        ...
        <TabsContent value="security" className="space-y-6">
          <Card className="bg-slate-900/40 border-slate-800 backdrop-blur-sm rounded-3xl overflow-hidden">
            <CardHeader className="border-b border-white/5 bg-white/5">
              ...
              <CardDescription className="text-slate-500">Update your password to keep your administrative account secure.</CardDescription>
            </CardHeader>
            <CardContent className="pt-8">
              <form onSubmit={handlePasswordSubmit} className="space-y-5">
                <div className="space-y-2">
                  <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">Current Password</Label>
                  <Input 
                    type="password"
                    value={passwordForm["current-password"]}
                    onChange={e => setPasswordForm({ ...passwordForm, "current-password": e.target.value })}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">New Password</Label>
                    <Input 
                      type="password"
                      value={passwordForm["new-password"]}
                      onChange={e => setPasswordForm({ ...passwordForm, "new-password": e.target.value })}
                      className="bg-slate-950 border-slate-800 rounded-xl h-11"
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">Confirm New Password</Label>
                    <Input 
                      type="password"
                      value={passwordForm["confirm-password"]}
                      onChange={e => setPasswordForm({ ...passwordForm, "confirm-password": e.target.value })}
                      className="bg-slate-950 border-slate-800 rounded-xl h-11"
                      required
                    />
                  </div>
                ...
                <div className="pt-4 flex items-center gap-4 bg-lime-400/5 p-4 rounded-2xl border border-lime-400/10">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">Changing your password will immediately terminate any other active sessions. You will need to log back in on other devices.</p>
                </div>
                <div className="pt-2">
                  <Button 
                    type="submit"
        ...
        <TabsContent value="system" className="space-y-6">
          <Card className="bg-slate-900/40 border-slate-800 backdrop-blur-sm rounded-3xl overflow-hidden">
            <CardHeader className="border-b border-white/5 bg-white/5">
              ...
              <CardDescription className="text-slate-500">Configure core system behavior and posting channels.</CardDescription>
            </CardHeader>
            <CardContent className="pt-8 space-y-6">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="bg-lime-400/10 border-lime-400/20 text-lime-400">
                  Native Count: {globalRoutingQuery.data?.modeCounts?.FORCE_NATIVE ?? "—"}
                </Badge>
                <Badge variant="outline" className="bg-blue-400/10 border-blue-400/20 text-blue-400">
                   Upload-Post: {globalRoutingQuery.data?.modeCounts?.FORCE_UPLOAD_POST ?? "—"}
                </Badge>
              </div>

              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">Default Posting Channel</Label>
                  <Select
                    value={effectiveGlobalDefault.mode}
                    onChange={(e) => setDraftGlobalDefault({ ...effectiveGlobalDefault, mode: e.target.value as RoutingMode })}
                    className="bg-slate-950 border-slate-800 rounded-xl h-11"
                  >
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-black uppercase tracking-widest text-slate-500 pl-1">Apply To Scope</Label>
                  <Select
                    value={effectiveApplyTo}
                    onChange={(e) => setUserChosenScope(e.target.value as ApplyScope)}
                    className="bg-slate-950 border-slate-800 rounded-xl h-11"
                  >
              ...
              {effectiveGlobalDefault.mode === "FORCE_UPLOAD_POST" && (
                <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-6 space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Platform Toggles (Upload-Post Only)</p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    {[
                      { id: "useInstagram", label: "Instagram", field: "useInstagram" },
                      { id: "useFacebook", label: "Facebook", field: "useFacebook" },
                      { id: "useLinkedin", label: "LinkedIn", field: "useLinkedin" },
                    ].map(p => (
                      <label key={p.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-800 bg-slate-900 group cursor-pointer hover:border-lime-400/50 transition-all">
                        <span className="text-sm font-bold text-slate-300 group-hover:text-white">{p.label}</span>
                        <Checkbox 
                          checked={(effectiveGlobalDefault as any)[p.field]}
                          onCheckedChange={val => setDraftGlobalDefault({ ...effectiveGlobalDefault, [p.field]: !!val })}
              ...
              <div className="pt-4">
                ...
                >
                  {applyGlobalMutation.isPending ? "Applying..." : `Apply Globally to ${effectiveApplyTo === "ALL_USERS" ? "Everyone" : "Users"}`}
                </Button>
              </div>
            </CardContent>
          </Card>
